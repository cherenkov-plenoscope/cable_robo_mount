...
def light_field_sensor_portal(
    ...
    xml = ''
    xml += '<light_field_sensor>\n'
    xml += '<set_frame name="'+name+'" pos="'+tuple3(pos)+'" rot="'+tuple3(rot)+'"/>\n'
    xml += '<set_light_field_sensor\n'
    xml += 'expected_imaging_system_focal_length="106.5"\n'
    xml += 'expected_imaging_system_aperture_radius="35.5"\n'
    xml += 'max_FoV_diameter_deg="6.5"\n'
    xml += 'hex_pixel_FoV_flat2flat_deg="0.066667"\n'
    xml += 'housing_overhead="1.1"\n'
    xml += 'number_of_paxel_on_pixel_diagonal="9"\n'
    xml += 'lens_refraction_vs_wavelength="'+lens_refraction_vs_wavelength+'"\n'
    xml += 'bin_reflection_vs_wavelength="'+mirror_reflectivity_vs_wavelength+'"\n'
    xml += '/>\n'
    xml += '</light_field_sensor>\n'
    return xml
...
def triangle_xml(name, pos, rot, Ax, Ay, Bx, By, Cx, Cy, color, refl):
    xml = '<triangle>\n'
    xml += '    <set_frame name="'+name+'" '
    xml += 'pos="' + tuple3(pos) + '" '
    xml += 'rot="' + tuple3(rot) + '"/>\n'
    xml += '    <set_surface reflection_vs_wavelength="'+refl+'" '
    xml += 'color="'+color+'"/>\n'
    xml += '    <set_triangle '
    xml += 'Ax="'+float2str(Ax)+'" '
    xml += 'Ay="'+float2str(Ay)+'" '
...
def dish_support_concrete_pillar_xml(
    name,
    pos,
    rot,
    height,
...
def visual_scenery(reflector, number_of_dish_pillars=9):
    ...
    xml += constant_function(name='reflection_vs_wavelength', value=1.0)
    xml += constant_function(name='refraction_vs_wavelength', value=1.45)
    xml += color('desert_sand', [204, 102, 0])
    xml += color(name='facet_color', rgb=np.array([75, 75, 75]))
    xml += color(name='pale_blue_white', rgb=np.array([225, 255, 255]))
    xml += color(name='cable_color', rgb=np.array([140, 128, 128]))
