...
from .SAP2000_bridge.HomTra_bridge_tools import (
    get_nodes_moved_position,
    get_nodes_zenith_position,
)
...
def brute_force():
    ...
    brute_force_specs_path = os.path.join(
        brute_force_specs["working_directory"], "brute_force_specs.json"
    )
    config.write(brute_force_specs, brute_force_specs_path)

    for i in range(
        int(brute_force_specs["start_value"]),
        int(brute_force_specs["fin_value"]) + 1,
        int(brute_force_specs["step"]),
    ):
        ...
        output_path = os.path.join(
            brute_force_specs["working_directory"], str(run_number)
        )
        ...
        results = run(
            working_directory=brute_force_specs["working_directory"],
            cfg=cfg,
...
def brute_force2():
    ...
    brute_force_specs = {
        ...
        "working_directory": "C:\\Users\\Spiros Daglas\\Desktop\\run\\dish50_BF_ang45_barsthickness",
    }  ###change here
    ...
    for i in range(
        int(brute_force_specs["start_value1"]),
        (
            int(brute_force_specs["fin_value1"])
            + int(brute_force_specs["step1"])
        ),
        int(brute_force_specs["step1"]),
    ):
        for j in range(
            int(brute_force_specs["start_value2"]),
            (
                int(brute_force_specs["fin_value2"])
                + int(brute_force_specs["step2"])
            ),
            int(brute_force_specs["step2"]),
        ):
            ...
            cfg["reflector"]["bars"]["thickness"] = (
                float(i) / 1000
            )  ###change here
            cfg["tension_ring"]["bars"]["thickness"] = (
                float(j) / 1000
            )  ###change here
            # cfg['tension_ring']['bars']['outer_diameter'] = float(k)*0.0001 ###change here

            run_number = current_run_number(
                brute_force_specs["working_directory"]
            )
            output_path = os.path.join(
                brute_force_specs["working_directory"], str(run_number)
            )
            ...
            variables_vector = {
                "x_over_z_ratio": float(i) / 1000,
                "inner_hex_radius": float(j) / 1000,
            }  ###change here
            variables_vector_path = os.path.join(
                output_path, "variables_vector.json"
            )
            config.write(variables_vector, variables_vector_path)

            results_path = os.path.join(
                output_path, "intermediate_results.json"
            )
            results = run(
                working_directory=brute_force_specs["working_directory"],
                cfg=cfg,
...
def current_run_number(working_directory):
    ...
    run_numbers_in_working_directory = np.array(
        run_numbers_in_working_directory
    )
...
def estimate_optical_performance(cfg, dish, alignment, output_path):
    ...
    mctracer_propagate_path = cfg["system"]["merlict"][
    ...
    mctracer_bridge.write_reflector_xml(
        reflector=dish, alignment=alignment, path=mct_scenery_path
    )
    ...
    camera_res = mctracer_bridge.star_light_analysis.read_binary_response(
        mct_camera_response_path
    )
    ground_res = mctracer_bridge.star_light_analysis.read_binary_response(
        mct_ground_response_path
    )
    stddev_of_psf = (
        mctracer_bridge.star_light_analysis.stddev_of_point_spread_function(
            camera_res
        )
    )
    image = (
        mctracer_bridge.star_light_analysis.make_image_from_sensor_response(
            dish, camera_res, cfg["star_light_analysis"]
        )
    )
    ground_image = (
        mctracer_bridge.star_light_analysis.make_image_from_ground_response(
            dish, ground_res, cfg["star_light_analysis"]
        )
    )

    mctracer_bridge.star_light_analysis.save_image(
        image, os.path.join(output_path, "camera_image.png")
    )
    mctracer_bridge.star_light_analysis.save_image(
        ground_image, os.path.join(output_path, "ground_image.png")
    )
...
def estimate_deformed_nodes(structural, dish, windBOOL):
    ...
    TextFilesBridge.FramesCreate(
        dish["bars_reflector"], dish["bars_tension_ring"], structural
    )
...
def run(working_directory, cfg, output_path):
    ...
    zenith_dish["nodes"] = estimate_deformed_nodes(
        structural, initial_dish, False
    )
    ...
    homogenous_transformation.set_translation(
        geometry.translational_vector_xyz
    )
    ...
    deformed_transformed_dish["nodes"] = estimate_deformed_nodes(
        structural, deformed_transformed_dish, True
    )
    ...
    return {
        ...
        "reflector_bars_length": float(
            tools.bars_length(
                initial_dish["nodes"], initial_dish["bars_reflector"]
            ).sum()
        ),
        "tension_ring_bars_length": float(
            tools.bars_length(
                initial_dish["nodes"], initial_dish["bars_tension_ring"]
            ).sum()
        ),
        "reflector_weight": float(
            structural.reflector_material_specific_weight
            / 10
            * structural.bars_reflector_cs_area
            * (
                tools.bars_length(
                    initial_dish["nodes"], initial_dish["bars_reflector"]
                ).sum()
        ...
        "tension_ring_weight": float(
            structural.tension_ring_material_specific_weight
            / 10
            * structural.bars_tension_ring_cs_area
            * (
                tools.bars_length(
                    initial_dish["nodes"], initial_dish["bars_tension_ring"]
                ).sum()
        ...
        "max_intial_deformation": np.linalg.norm(
            zenith_dish["nodes"] - initial_dish["nodes"], axis=1
        ).max(),
        "max_final_deformation": np.linalg.norm(
            deformed_dish["nodes"] - initial_dish["nodes"], axis=1
        ).max(),
        "max_in_plane_final_deformation": np.linalg.norm(
            (deformed_dish["nodes"] - initial_dish["nodes"])[:, [0, 1]], axis=1
        ).max(),
    }
