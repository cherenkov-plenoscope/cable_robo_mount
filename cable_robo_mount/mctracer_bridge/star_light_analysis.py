...
def make_image_from_sensor_response(
    reflector, sensor_response, analysis_config
):
    ...
    bins = np.linspace(-2 * roi, +2 * roi, number_of_bins)
    image = np.histogram2d(x=theta_x, y=theta_y, bins=[bins, bins])[0]
    return {"histogram": image, "bins": bins, "unit": "mdeg"}


def make_image_from_ground_response(
    reflector, ground_response, analysis_config
):
    ...
    bins = np.linspace(-reflector_radius, +reflector_radius, number_of_bins)
    image = np.histogram2d(
        x=ground_response["x"], y=ground_response["y"], bins=bins
    )[0]
    return {"histogram": image, "bins": bins, "unit": "m"}
