...
def cylinder(name, start_pos, end_pos, radius, color, refl="zero"):
    return {
        ...
        "radius": float(radius),
        "surface": {"outer_color": color, "outer_reflection": refl},
        "children": [],
    }
...
def dish_support_concrete_pillar(name, pos, rot, height, width, color, refl):
    return {
        "type": "Frame",
        "name": name,
        "pos": list_f8(pos),
...
def facets_to_merlict(reflector, alignment):
    reflector2facets = mirror_alignment.reflector2facets(
        reflector=reflector, alignment=alignment
    )
    facets = []
...
def benchmark_scenery(reflector, alignment):
    ...
    sc["functions"] = [
        {
            "name": "zero",
            "argument_versus_value": [[200e-9, 0.0], [1200e-9, 0.0]],
        },
        {
            "name": "reflection_vs_wavelength",
            "argument_versus_value": [[200e-9, 0.9], [1200e-9, 0.9]],
        },
    ]
    sc["colors"] = [
...
def write_reflector(reflector, alignment, path):
    ...
    """
    write_json(benchmark_scenery(reflector, alignment), path)
...
def write_star_light(reflector, path, number_of_photons=1e6):
    ...
    write_json(
        star_light(
            ...
            pos=np.array(
                [0.0, 0.0, 10.0 * reflector["geometry"].focal_length]
            ),
...
def write_camera_tower(tower_nodes_and_bars, name, pos, rot, color="white"):
    children = []
...
def visual_scenery(reflector, number_of_dish_pillars=9):
    ...
    reflector["nodes"] = transform_nodes(
        reflector["nodes"], geometry.root2dish
    )
    ...
    sc["functions"] = [
        ...
        {
            "name": "reflection_vs_wavelength",
            "argument_versus_value": [[200e-9, 1.0], [1200e-9, 1.0]],
        },
        {
            "name": "refraction_vs_wavelength",
            "argument_versus_value": [[200e-9, 1.45], [1200e-9, 1.45]],
        },
    ]
    sc["colors"] = [
        {"name": "facet_color", "rgb": [75, 75, 75]},
    ...
    tmp_facets = facets_to_merlict(
        reflector=reflector, alignment=ideal_alignment
    )
    for tmp_facet in tmp_facets:
        sc["children"].append(tmp_facet)
    ...
    dish_pillar_az_angles = np.linspace(
        0, 2 * np.pi, number_of_dish_pillars, endpoint=False
    )

    for phi in dish_pillar_az_angles:
        r = dish_pillar_width / 2 + dish_pillar_circle_radius
        dish_pillar_positions = [r * np.cos(phi), r * np.sin(phi), 0.0]
    ...
    for tower_position in tower_positions:
        ...
        for base_position in base_positions:
            absolute_base_position = np.array(base_position) + np.array(
                tower_position
            )
            sc["children"].append(
                cylinder_mount(
    ...
    camera_nodes_in_root_frame = transform_nodes(
        nodes=camera_structure["nodes"], trafo=geometry.root2camera
    )
    ...
    for i, tower_position in enumerate(tower_positions):
        upper_tower_cable_node = np.array(tower_position) + np.array(
            [0, 0, tower_hight]
        )
        lower_tower_cable_node = np.array(tower_position) + np.array(
            [0, 0, tower_hight * 0.45]
        )
