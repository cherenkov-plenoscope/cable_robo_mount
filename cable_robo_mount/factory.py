...
def bar_is_part_of_reflector_dish(bar, nodes, geometry):
    ...
    return (
        radii.max()
        <= geometry.max_outer_radius + geometry.facet_outer_hex_radius
    )
...
def mirror_tripod_is_part_of_reflector_dish(
    mirror_tripod, nodes, geometry, outer_shape="hexagon"
):
    ...
    if A_in_range and B_in_range and C_in_range:
        ...
        if outer_shape == "round":
            ...
            inside_outer_limit = (
                radius + geometry.facet_outer_hex_radius
            ...
            inside_inner = point_in_hexagon(
                ...
                y=center[1],
                inner_radius=(
                    geometry.min_inner_radius + geometry.facet_outer_hex_radius
                ),
                phi=0,
            )
                ...
                if k == 0:  # only on top layer
                    if np.mod(i, 2) == 0:  # Each 2nd in i
                        if np.mod(j + 1, 2) == 0:  # Each 2nd in j
                            ...
                            if mirror_tripod_is_part_of_reflector_dish(
                                mirror_tripod, nodes, geometry
                            ):
...
def generate_reflector_with_tension_ring_and_cables(geometry):
    ...
    cables_structure = generate_cables(
        geometry,
        tension_ring["nodes_reflector_tension_ring"],
        tension_ring["elastic_supports"],
    )
    all_nodes = np.concatenate(
        (
            reflector["nodes"],
            tension_ring["nodes_tension_ring_only_new"],
            cables_structure["nodes"],
        ),
        axis=0,
    )
    return {
