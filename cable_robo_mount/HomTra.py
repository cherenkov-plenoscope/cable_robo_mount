...
class HomTra(object):
    ...
    def multiply(self, mul):
        ...
        out = HomTra()
        out.T[0, 0] = (
            T[0, 0] * G[0, 0] + T[0, 1] * G[1, 0] + T[0, 2] * G[2, 0]
        )  # + T[0,3]*      0.0
        out.T[0, 1] = (
            T[0, 0] * G[0, 1] + T[0, 1] * G[1, 1] + T[0, 2] * G[2, 1]
        )  # + T[0,3]*      0.0
        out.T[0, 2] = (
            T[0, 0] * G[0, 2] + T[0, 1] * G[1, 2] + T[0, 2] * G[2, 2]
        )  # + T[0,3]*      0.0
        out.T[0, 3] = (
            T[0, 0] * G[0, 3] + T[0, 1] * G[1, 3] + T[0, 2] * G[2, 3] + T[0, 3]
        )  # *      1.0

        out.T[1, 0] = (
            T[1, 0] * G[0, 0] + T[1, 1] * G[1, 0] + T[1, 2] * G[2, 0]
        )  # + T[1,3]*      0.0
        out.T[1, 1] = (
            T[1, 0] * G[0, 1] + T[1, 1] * G[1, 1] + T[1, 2] * G[2, 1]
        )  # + T[1,3]*      0.0
        out.T[1, 2] = (
            T[1, 0] * G[0, 2] + T[1, 1] * G[1, 2] + T[1, 2] * G[2, 2]
        )  # + T[1,3]*      0.0
        out.T[1, 3] = (
            T[1, 0] * G[0, 3] + T[1, 1] * G[1, 3] + T[1, 2] * G[2, 3] + T[1, 3]
        )  # *      1.0

        out.T[2, 0] = (
            T[2, 0] * G[0, 0] + T[2, 1] * G[1, 0] + T[2, 2] * G[2, 0]
        )  # + T[2,3]*      0.0
        out.T[2, 1] = (
            T[2, 0] * G[0, 1] + T[2, 1] * G[1, 1] + T[2, 2] * G[2, 1]
        )  # + T[2,3]*      0.0
        out.T[2, 2] = (
            T[2, 0] * G[0, 2] + T[2, 1] * G[1, 2] + T[2, 2] * G[2, 2]
        )  # + T[2,3]*      0.0
        out.T[2, 3] = (
            T[2, 0] * G[0, 3] + T[2, 1] * G[1, 3] + T[2, 2] * G[2, 3] + T[2, 3]
        )  # *      1.0
    ...
    def inverse(self):
        ...
        out.T[0, 2] = T[2, 0]
        out.T[0, 3] = -(
            T[0, 0] * T[0, 3] + T[1, 0] * T[1, 3] + T[2, 0] * T[2, 3]
        )
        ...
        out.T[2, 2] = T[2, 2]
        out.T[2, 3] = -(
            T[0, 2] * T[0, 3] + T[1, 2] * T[1, 3] + T[2, 2] * T[2, 3]
        )
        return out
