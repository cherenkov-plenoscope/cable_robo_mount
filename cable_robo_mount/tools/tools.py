...
def angle_between_bars(bar1, bar2, nodes):
    ...
    return np.arccos(
        sum(Vec1.T * Vec2.T) / (np.linalg.norm(Vec1) * np.linalg.norm(Vec2))
    )
...
def list_of_angles_between_neighbouring_bars(joints, bars, nodes):
    ...
    for i in range(len(joints)):
        for j in range(len(joints[i]) - 1):
            ...
            end_list.append(
                [angle_between_bars(bar1=bar1, bar2=bar2, nodes=nodes)]
            )
    return end_list
...
def mirror_tripod_centers(nodes, mirror_tripods):
    tripod_centers = np.zeros(
        shape=(mirror_tripods.shape[0], 3), dtype=np.float64
    )
