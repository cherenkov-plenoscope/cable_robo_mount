...
def image_collector(
    base_directory, output_directory="C:\\Users\\Spiros Daglas\\Desktop\\pics"
):
    directories = results_for_case(base_directory)
    for count, dir_number in enumerate(directories):
        initial_image_path = os.path.join(
            base_directory, str(dir_number), "camera_image.png"
        )
        final_image_path = os.path.join(
            output_directory, "camera_image" + str(count) + ".png"
        )
        try:
...
def save_int_res():
    for dict_key in [
        "stddev_of_psf",
        "max_final_deformation",
        "max_intial_deformation",
        "max_in_plane_final_deformation",
        "reflector_weight",
        "tension_ring_weight",
    ]:
        create_history_of_data(
            directory=directory,
            file_name="intermediate_results.json",
            dict_key=dict_key,
        )


def create_fitness_history_of_data():
    fitness_function = collect_data(
        directory=directory,
        file_name="intermediate_results.json",
        dict_key="fitness_function",
    )
    ...
    tension_ring_weight = (
        collect_data(
            directory=directory,
            file_name="intermediate_results.json",
            dict_key="tension_ring_weight",
        )
        * w_tension_ring_weight
    )
    max_final_deformation = (
        collect_data(
            directory=directory,
            file_name="intermediate_results.json",
            dict_key="max_final_deformation",
        )
        * w_max_final_deformation
    )
    plt.ion()
    g1 = plt.plot(
        np.arange(len(fitness_function)),
        fitness_function,
        color="black",
        linewidth=2.0,
    )[0]
    ...
    g3 = plt.plot(
        np.arange(len(reflector_weight)), reflector_weight, color="b"
    )[0]
    g4 = plt.plot(
        np.arange(len(tension_ring_weight)), tension_ring_weight, color="g"
    )[0]
    g5 = plt.plot(
        np.arange(len(max_final_deformation)), max_final_deformation, color="m"
    )[0]
    ax = plt.gca()
...
def fit_curve_to_data(data):
    ...
    plt.plot(
        x * data.shape[0] / 4,
        fitFunc(x, fitParams[0], fitParams[1], fitParams[2]) * 100,  ##change
        x * data.shape[0] / 4,
        fitFunc(
            x,
            fitParams[0] + sigma[0],
...
def create_history_of_data(
    ...
    plt.title(
        "Progress of PSO. Best iteration:"
        + str(iter_number_of_min_dict_data)
        + " with "
        + dict_key
        + ": "
        + str(min_dict_data)
...
def collect_pareto_points(
    directory=directory, file_name="variables_vector.json"
):
    ...
    for i in range(variables_number):
        pareto_points.append(
            collect_data(
                directory=directory, file_name=file_name, dict_key=i
            ).tolist()
        )
...
def simple_cull(pts, dominates):
    ...
    while remaining:
        ...
        for other in remaining[1:]:
            [new_remaining, dominated][dominates(candidate, other)].append(
                other
            )
