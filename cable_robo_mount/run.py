...
def PSO():
    ...
    pso_specs = {
        "lower_bounds": [
            0.1054
        ],  # 0.0713, 0.000521, 0.006, 0.006, 1.36, 0.9, 0.6],  ###change here
        "upper_bounds": [
            0.10541
        ],  # 0.163, 0.001322, 0.01, 0.0105, 2.26, 1.5, 1.2], ###change here
    ...
    pso_specs_path = os.path.join(
        pso_specs["working_directory"], "pso_specs.json"
    )
    config.write(pso_specs, pso_specs_path)

    xopt, fopt, p, fp = pyswarm.pso(
        run,
        lb=pso_specs["lower_bounds"],
        ub=pso_specs["upper_bounds"],
        swarmsize=int(pso_specs["swarmsize"]),
        maxiter=int(pso_specs["maxiter"]),
        debug=True,
        particle_output=True,
        kwargs={"working_directory": pso_specs["working_directory"]},
    )
...
def make_run_config(var_vector, template_config):
    ...
    run_config["reflector"]["bars"]["outer_diameter"] = var_vector[
        0
    ]  ###change here
    ...
    return {"run_config": run_config, "var_vector": var_vector}
...
def run(var_vector, working_directory, template_config=config.example):
    ...
    variables_vector = make_run_config(var_vector, template_config)[
        "var_vector"
    ]
    ...
    fitness_function = (
        stddev_of_psf / 2
        + reflector_weight / 250
        + tension_ring_weight / 150
        + max_final_deformation / 2
    )  ###change here
