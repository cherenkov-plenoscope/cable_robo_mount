...
def generate_camera_space_frame_quint(light_field_sensor_radius=5.0):
    ...
    nodes = np.array(
        [
            # lower layer
            [
                r * np.cos(2 * np.pi * 0.0 + off),
                r * np.sin(2 * np.pi * 0.0 + off),
                0,
            ],  # 0
            [
                r * np.cos(2 * np.pi * 0.2 + off),
                r * np.sin(2 * np.pi * 0.2 + off),
                0,
            ],  # 1
            [
                r * np.cos(2 * np.pi * 0.4 + off),
                r * np.sin(2 * np.pi * 0.4 + off),
                0,
            ],  # 2
            [
                r * np.cos(2 * np.pi * 0.6 + off),
                r * np.sin(2 * np.pi * 0.6 + off),
                0,
            ],  # 3
            [
                r * np.cos(2 * np.pi * 0.8 + off),
                r * np.sin(2 * np.pi * 0.8 + off),
                0,
            ],  # 4
            # upper layer
            [
                r * np.cos(2 * np.pi * 0.1 + off),
                r * np.sin(2 * np.pi * 0.1 + off),
                r,
            ],  # 5
            [
                r * np.cos(2 * np.pi * 0.3 + off),
                r * np.sin(2 * np.pi * 0.3 + off),
                r,
            ],  # 6
            [
                r * np.cos(2 * np.pi * 0.5 + off),
                r * np.sin(2 * np.pi * 0.5 + off),
                r,
            ],  # 7
            [
                r * np.cos(2 * np.pi * 0.7 + off),
                r * np.sin(2 * np.pi * 0.7 + off),
                r,
            ],  # 8
            [
                r * np.cos(2 * np.pi * 0.9 + off),
                r * np.sin(2 * np.pi * 0.9 + off),
                r,
            ],  # 9
            # central node
            [0, 0, 1.5 * r],  # 10
...
def generate_camera_space_frame(
    light_field_sensor_radius=5.0, camera_housing_hight=1.0
):
    """
...
def generate_camera_tower(
    hight=161, top_width=4.2, base_width=19.6, bar_radius=None
):
    """
    ...
    return {"nodes": nodes, "bars": bars, "bar_radius": bar_radius}
