...
total_geometry = rs.factory.generate_reflector_with_tension_ring_and_cables(
    geometry
)
...
nodes_rotated = rs.SAP2000_bridge.HomTra_bridge_tools.get_nodes_moved_position(
    nodes, cable_supports, homogenous_transformation
)
...
rs.SAP2000_bridge.TextFilesBridge.FramesCreate(
    bars_reflector, bars_tension_ring, structural
)
...
nodes_deformed_rotated = (
    bridge.get_total_absolute_deformations_for_load_combination(
        nodes=nodes_rotated,
...
"""
nodes_deformed = (
    rs.SAP2000_bridge.HomTra_bridge_tools.get_nodes_zenith_position(
        nodes_deformed_rotated, cable_supports, homogenous_transformation
    )
)
...
mctracer_propagate_path = rs.config.example["system"]["merlict"][
...
rs.mctracer_bridge.write_star_light_xml(
    reflector=reflector_deformed, path="light.xml"
)
rs.mctracer_bridge.write_reflector_xml(
    reflector=reflector_deformed, alignment=alignment, path="scenery.json"
)
...
ground_res = rs.mctracer_bridge.star_light_analysis.read_binary_response(
    "out1_1"
)
image = rs.mctracer_bridge.star_light_analysis.make_image_from_sensor_response(
    reflector, res, rs.mctracer_bridge.star_light_analysis.config
)
shadow_image = (
    rs.mctracer_bridge.star_light_analysis.make_image_from_ground_response(
        reflector, ground_res, rs.mctracer_bridge.star_light_analysis.config
    )
)
end = time.time()
