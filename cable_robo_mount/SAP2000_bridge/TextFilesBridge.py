...
class JointsCreate(object):
    def __init__(self, nodes, structural):
        ...
        self.final_s2k = self.import_table_to_text_file(
            self.table_joint_coordinates, self.s2k_contents
        )
    ...
    def create_table_nodes(self, nodes):
        ...
        for i in range(nodes.shape[0]):
            table_joint_coordinates.append(
                ...
                + "   Z="
                + str(nodes[i, 2])
                + "   SpecialJt=Yes   GUID=e91ddf1b-1450-4830-ae89-2eec09d30794\n"
            )
        table_joint_coordinates.append(" \n")
...
class FramesCreate(object):
    def __init__(self, bars_reflector, bars_tension_ring, structural):
        ...
        self.file_path = structural.SAP_2000_working_directory + ".$2k"
        self.s2k_contents = self.text_file_to_list(self.file_path)
        self.tables_1_frames, self.tables_2_frames = self.create_table_frames(
            bars_reflector, bars_tension_ring
        )
        self.final_s2k_w2 = self.import_table_2_to_text_file(
            self.tables_2_frames, self.s2k_contents
        )
        self.final_s2k_w1 = self.import_table_1_to_text_file(
            self.tables_1_frames, self.final_s2k_w2
        )
    ...
    def create_table_frames(self, bars_reflector, bars_tension_ring):
        ...
        for i in range(bars.shape[0]):
            tables_1.append(
                ...
                + "   JointJ=node_"
                + str(bars[i, 1])
                + "   IsCurved=No   GUID=9752f2a1-3a94-4f86-b5ea-81a589ee9daa\n"
            )
        tables_1.append(" \n")
        ...
        self.property_name_tension_ring = (
            "ROR_"
