...
class Bridge(object):
    def __init__(self, structural):
        ...
        self._SapObject = comtypes.client.GetActiveObject(
    ...
    def material_definition_steel(self, part_of_structure_as_string):
        if part_of_structure_as_string == "reflector":
            material_name = (
                "Steel_S"
                + str(self.structural.reflector_yielding_point / 1000)
                + "SpaceFrameBars"
            )
            ...
            specific_weight = (
                self.structural.reflector_material_specific_weight
            )
        elif part_of_structure_as_string == "tension_ring":
            material_name = (
                "Steel_S"
                + str(self.structural.tension_ring_yielding_point / 1000)
                + "TensionRingBars"
            )
            ...
            specific_weight = (
                self.structural.tension_ring_material_specific_weight
            )
        self._SapModel.PropMaterial.SetMaterial(
            Name=material_name, MatType=1, Color=-1, Notes="custom-made"
        )
        self._SapModel.PropMaterial.SetMPIsotropic(
            Name=material_name, E=e_modul, U=0.3, A=1.170e-05
        )
        ...
        self._SapModel.PropMaterial.SetWeightAndMass(
            Name=material_name, MyOption=1, Value=specific_weight
        )
    ...
    def pipe_cross_section_definition(self, part_of_structure_as_string):
        if part_of_structure_as_string == "reflector":
            property_name = (
                "ROR_"
                + str(1000 * self.structural.reflector_bar_outer_diameter)
                + "x"
                + str(1000 * self.structural.reflector_bar_thickness)
                + "SpaceFrameBars"
            )
            material_name = (
                "Steel_S"
            ...
            property_name = (
                "ROR_"
                + str(1000 * self.structural.tension_ring_bar_outer_diameter)
                + "x"
                + str(1000 * self.structural.tension_ring_bar_thickness)
                + "TensionRingBars"
            )
            material_name = (
                "Steel_S"
    ...
    def _shell_definition(self):
        self._SapModel.PropArea.SetShell_1(
            ...
            MatProp="Steel_S"
            + str(self.structural.reflector_yielding_point / 1000)
            + "SpaceFrameBars",  # ignored in any case
            MatAng=0.0,
            Thickness=0.0,
            Bending=0.0,
            Color=-1,
    ...
    def _set_tension_compression_limits_for_specific_frame_elements(
        self, cables
    ):
    ...
    def _cables_definition(self, cables):
        property_name = (
            "Steel_Y"
            + str(self.structural.cables_yielding_point / 1000)
            + "TensionRingCables"
        )

        self._SapModel.PropMaterial.SetMaterial(
            Name=property_name, MatType=1, Color=-1
        )
        ...
        self._SapModel.PropCable.SetModifiers(
            Name=property_name, Value=CSarea_mass_weight_modifiers
        )
    ...
    def elastic_support_definition(self, fixtures):
        ...
        for i in range((fixtures.shape[0])):
            self._SapModel.PointObj.SetSpring(
                "node_" + str(fixtures[i]), spring_stiffness
            )
    ...
    def _material_property_modifiers(self, material_name="densless_shell"):
        ...
        self._SapModel.PropArea.SetModifiers(
            Name=material_name, Value=stiffness_mass_weight_modifiers
        )

    def _area_object_with_points_definition(
        self, mirror_tripods, points_number=3
    ):
        nodes_of_mirror_tripod = []
        for i in range(len(mirror_tripods)):
            nodes_of_mirror_tripod.append([])
            for j in range(len(mirror_tripods[i])):
                nodes_of_mirror_tripod[i].append(
                    "node_" + str(mirror_tripods[i, j])
                )
    ...
    def load_scenario_dead(self, load_pattern_name="dead_load"):
        self._SapModel.LoadPatterns.Add(
            Name=load_pattern_name, MyType=1, SelfWTMultiplier=1
        )

    def load_scenario_facet_weight(
        self, mirror_tripods, load_pattern_name="facets_live_load"
    ):
        self._SapModel.LoadPatterns.Add(
            Name=load_pattern_name, MyType=3, SelfWTMultiplier=0
        )
            ...
            for j in range((mirror_tripods.shape[1])):
                self._SapModel.PointObj.SetLoadForce(
                    ...
                    Value=[
                        0,
                        0,
                        -self.structural.tripod_nodes_weight,
                        0,
                        0,
                        0,
                    ],  # in each DOF
    ...
    def load_scenario_wind(
        self, mirror_tripods, nodes, load_pattern_name="wind"
    ):
        self._SapModel.LoadPatterns.Add(
            Name=load_pattern_name, MyType=6, SelfWTMultiplier=0
        )
        ...
        self._SapModel.LoadPatterns.AutoWind.SetEurocode12005_1(
            ...
            BottomZ=np.amin(nodes, axis=0)[2]
            - self.structural.reflector_security_distance_from_ground,
    ...
    def load_combination_3LP_definition(
        self,
        structural,
        CName1="dead_load",
        CName2="facets_live_load",
        CName3="wind",
        load_combination_name="dead+live+wind",
    ):
        self.load_combination_name = load_combination_name
        self._SapModel.RespCombo.Add(Name=load_combination_name, ComboType=0)
        self._SapModel.RespCombo.SetCaseList(
            load_combination_name,
            0,
            CName1,
            structural.dead_load_scenario_security_factor,
        )
        self._SapModel.RespCombo.SetCaseList(
            load_combination_name,
            0,
            CName2,
            structural.live_load_scenario_security_factor,
        )
        self._SapModel.RespCombo.SetCaseList(
            load_combination_name,
            0,
            CName3,
            structural.wind_load_scenario_security_factor,
        )

    def load_combination_2LP_definition(
        self,
        structural,
        CName1="dead_load",
        CName2="facets_live_load",
        load_combination_name="dead+live",
    ):
        self.load_combination_name = load_combination_name
        self._SapModel.RespCombo.Add(Name=load_combination_name, ComboType=0)
        self._SapModel.RespCombo.SetCaseList(
    ...
    def non_linearity(self):
        ...
        self._SapModel.LoadCases.StaticNonLinear.SetCase(
            "nonlinear_incorporation"
        )
        self._SapModel.LoadCases.StaticNonlinear.SetGeometricNonlinearity(
            "dead_load", 2
        )
        self._SapModel.LoadCases.StaticNonlinear.SetGeometricNonlinearity(
            "facets_live_load", 2
        ...
        self._SapModel.LoadCases.StaticNonlinear.SetGeometricNonlinearity(
            "nonlinear_incorporation", 2
        )
        self._SapModel.LoadCases.StaticNonLinear.SetLoads(
            "nonlinear_incorporation",
            2,
            ["Load", "Load"],
            ["dead_load", "facets_live_load"],
            [1.0, 1.0],
        )
        self._SapModel.RespCombo.Add(
            Name="nonlinear_incorporation_combo", ComboType=0
        )
        self._SapModel.RespCombo.SetCaseList(
            "nonlinear_incorporation_combo", 0, "nonlinear_incorporation", 1.0
        )
        self.load_combination_name = "nonlinear_incorporation_combo"

    def non_linearity_wind(self, mirror_tripods, nodes):
        self.load_scenario_wind(
            mirror_tripods, nodes, load_pattern_name="wind"
        )
        ...
        self._SapModel.LoadCases.StaticNonLinear.SetLoads(
            "wind", 1, ["Load"], ["wind"], [1.0]
        )
        self._SapModel.LoadCases.StaticNonlinear.SetInitialCase(
            "wind", "nonlinear_incorporation"
        )
    ...
    def get_displacements_for_group_of_nodes_for_selected_load_pattern(
        self, load_pattern_name, group_name
    ):
        self._SapModel.Results.Setup.DeselectAllCasesAndCombosForOutput()
        self._SapModel.Results.Setup.SetCaseSelectedForOutput(
            load_pattern_name
        )
        ...
        [
            ...
            R3,
            ret,
        ] = self._SapModel.Results.JointDispl(
            Name,
            mtypeElm,
            NumberResults,
            Obj,
        ...
        for i in range(len(Obj)):
            relative_displacements.append(
                [Obj[i], U1[i], U2[i], U3[i], R1[i], R2[i], R3[i]]
            )
        return relative_displacements

    def get_displacements_for_group_of_nodes_for_selected_load_combination(
        self, load_combination_name, group_name
    ):
        self._SapModel.Results.Setup.DeselectAllCasesAndCombosForOutput()
        self._SapModel.Results.Setup.SetComboSelectedForOutput(
            load_combination_name
        )
    ...
    def get_forces_for_group_of_bars_for_selected_load_pattern(
        self, load_pattern_name, group_name="ALL"
    ):
        self._SapModel.Results.Setup.DeselectAllCasesAndCombosForOutput()
        self._SapModel.Results.Setup.SetCaseSelectedForOutput(
            load_pattern_name
        ...
        [
            ...
            M3,
            ret,
        ] = self._SapModel.Results.FrameForce(
            Name,
            ItemTypeElm,
            NumberResults,
            Obj,
    ...
    def get_forces_for_group_of_bars_for_selected_load_combination(
        self, load_combination_name, dish, part_of_structure
    ):
        if part_of_structure == "reflector":
            ...
            for i in range(bars.shape[0]):
                self._SapModel.FrameObj.SetGroupAssign(
                    Name="bar_" + str(i), GroupName=part_of_structure
                )
            ...
            for i in range(
                dish["bars_reflector"].shape[0],
                dish["bars_reflector"].shape[0] + bars.shape[0],
            ):
                self._SapModel.FrameObj.SetGroupAssign(
                    Name="bar_" + str(i), GroupName=part_of_structure
                )
            ...
            for i in range(bars.shape[0]):
                self._SapModel.CableObj.SetGroupAssign(
                    Name="cable_" + str(i), GroupName=part_of_structure
                )
            group_name = part_of_structure
        ...
        return self.forces_arrange(
            NumberResults, Obj, P, V2, V3, T, M2, M3, part_of_structure
        )

    def forces_arrange(
        self, NumberResults, Obj, P, V2, V3, T, M2, M3, part_of_structure
    ):
        forces = []
        for i in range(NumberResults - 1):
            if Obj[i] != Obj[i + 1]:
                ...
                coord_1 = np.array(
                    self._SapModel.PointObj.GetCoordCartesian(points[0])
                )
                coord_2 = np.array(
                    self._SapModel.PointObj.GetCoordCartesian(points[1])
                )
                length = np.linalg.norm(coord_1 - coord_2)
                forces.append(
                    [Obj[i], length, P[i], V2[i], V3[i], T[i], M2[i], M3[i]]
                )
        if part_of_structure == "cables":
    ...
    def get_total_absolute_deformations_for_load_pattern(
        self, nodes, load_pattern_name, group_name="ALL"
    ):
        self.group_of_nodes_definition(
            group_name, flat_part_of_reflector=np.arange(nodes.shape[0])
        )
        relative_displacements = self.get_displacements_for_group_of_nodes_for_selected_load_pattern(
            load_pattern_name, group_name
        )
    ...
    def get_total_absolute_deformations_for_load_combination(
        self, nodes, load_combination_name, group_name="ALL"
    ):
        self.group_of_nodes_definition(
            group_name, flat_part_of_reflector=np.arange(nodes.shape[0])
        )
        relative_displacements = self.get_displacements_for_group_of_nodes_for_selected_load_combination(
            load_combination_name, group_name
        )
        nodes_deformed = np.zeros((nodes.shape[0], 3))
        for i in range(len(relative_displacements)):
            nodes_deformed[i, 0] = nodes[i, 0] + relative_displacements[i][1]
